...
- **ChatGPT** – für den Großteil der Entwicklung
- **Claude** – für die finalen Schritte, nachdem die ChatGPT-Tokens aufgebraucht waren

## Abgabe
- Repo: [praxis-app](https://github.com/Claudio-ux-creator/praxis-app) (öffentlich)
- Commit-Historie: mehrere Commits über den gesamten Entwicklungsverlauf (kein einzelner „Final"-Commit)

## Setup
1. Backend starten: `cd backend && npm run dev`
2. Frontend bauen (einmalig, bzw. nach jeder Änderung an `frontend/src`): `cd frontend && node build.cjs`
3. App öffnen: http://localhost:3000

> Hinweis: Das Frontend wird nicht per `npm run dev` (Vite-Dev-Server) betrieben, sondern per esbuild zu einer einzelnen `build/app.js` gebündelt und vom Backend unter Port 3000 mit ausgeliefert. Details siehe [Installation & Start](#installation--start) weiter unten.
...
- **No-Show-Management** - Automatischer täglicher Check markiert vergangene, nicht abgeschlossene/abgesagte Termine als No-Show; ab 2 No-Shows Erinnerung, ab 3 Online-Sperre (`is_blocked`, von MFA manuell aufhebbar)
...
- **Praxisschließungen** - Zeitraum für die gesamte Praxis; storniert automatisch betroffene Termine und benachrichtigt Patienten, Hinweisbanner im MFA-/Arzt-Dashboard
...
| GET | /api/patients/search | Live-Suche per Versichertennummer oder Name (Autovervollständigung, Duplikat-Erkennung) |
...
| Methode | Endpunkt | Beschreibung |
|---|---|---|
| POST | /api/appointments | Termin buchen (Praxisschließung/Abwesenheit/Sperre werden geprüft) |
...
| PATCH | /api/appointments/:id/confirm-series | Ganze Impfserie bestätigen |
...
| PATCH | /api/mfa/appointments/:id/move | Termin durch MFA verschieben (Konflikt-/Abwesenheitsprüfung) |
...
| GET | /api/practice-closures/upcoming | Aktuelle/nächste Schließung (Dashboard-Hinweis) |
| POST | /api/practice-closures | Schließung anlegen (storniert automatisch betroffene Termine + benachrichtigt Patienten) |
| PATCH | /api/practice-closures/:id | Schließung bearbeiten |
| DELETE | /api/practice-closures/:id | Schließung löschen |

### No-Show-Management

| Methode | Endpunkt | Beschreibung |
|---|---|---|
| POST | /api/mfa/run-no-show-check | Täglichen No-Show-Check manuell auslösen (läuft sonst automatisch alle 24h) |
...
| 0013 | MFA-Patientenfelder | Zusatzfelder für MFA-verwaltete Patienten |
| 0014 | Diagnosen-Historie | Änderungsnachvollziehbarkeit für Diagnosen (Spalten) |
...
| 0019 | Impf-Frage entfernt | Redundante Fragebogen-Frage für Impfungen entfernt |
| 0020 | Praxisschließungen | practice_closures-Tabelle (Start-/Enddatum, Grund) |
