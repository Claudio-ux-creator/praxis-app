...
export default function MFADashboard() {
  ...
  const [acuteSlots, setAcuteSlots] = useState(null);
  ...
  useEffect(() => {
    if (!loading) {
      ...
      get("/acute-slots?date=" + today).then((r) => {
  ...
  const handleCreatePrescription = async () => {
    const r = await post("/prescriptions", {
      insuranceNumber: fixPatient, medicationName: fixMedication, initiatedByMfaId: 1
    });
    if (r.success) { setShowPrescriptionDialog(false); setFixPatient(""); setFixMedication(""); loadData(); }
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="md:col-span-1">
          ...
          <CardContent className="space-y-4">
            {(!acuteSlots || !acuteSlots.slots || acuteSlots.slots.length === 0) && (
          ...
          <Card className="w-full max-w-sm">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex gap-2 justify-end pt-2">
                ...
                <Button onClick={handleCreatePrescription} disabled={!fixPatient || !fixMedication}>Anlegen</Button>
              </div>
            </CardContent>
          </Card>
        </div>
